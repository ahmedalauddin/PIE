...
class TreeMindMap extends React.Component {
  ...
  appendCircle = (nodeContainers, direction) => {
    let color = direction === "left" ? "green" : "black";
  ...
  drawTree = (svg, root, direction) => {
    ...
    //console.log("-margin.left = " + -margin.left + " left.x - margin.top = " + left.x - margin.top);
    ...
    */
    ...
    this.appendCircle(newNodeContainers);

    // The "foreignObject" object will display the name text on the node.
    this.appendText(newNodeContainers);
  ...
  loadData = (direction) => {
    ...
    */
    data = JSON.parse(this.state.jsonData);
  ...
  update = (svg) => {
    ...
    let rightData = this.loadData("right");
    let leftData = this.loadData("left");
    let rightTree = d3.hierarchy(rightData);
    let leftTree = d3.hierarchy(leftData);

    this.drawTree(svg, leftTree, "left");
    //this.drawTree(svg, rightTree, "right");
